import { 
  FaJs, FaReact, FaBootstrap, FaPhp, FaGithub, FaJava, FaHtml5, FaCss3Alt 
} from "react-icons/fa";
import { 
  SiTailwindcss, SiCodeigniter, SiMysql, SiArduino 
} from "react-icons/si";
        ...
        <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-6 sm:gap-10 w-full justify-items-center">
          {allSkills.map((skill, index) => (
            <div
              ...
              title={skill.name}
            >
              <span className="text-3xl sm:text-4xl md:text-5xl">{skill.icon}</span>
            </div>
          ))}
        </div>
      </div>
